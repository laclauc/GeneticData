...
MultBlocICL =function (data,z1,w1,a=4,b=1,C=c(1,1),e=rep(1,max(data$x))){
  ...
  nl=colSums(w);
  Nkl=nk%*%t(nl[-1]);
  nkl=array(unlist(lapply(1:r,function(h){t(z)%*%v_ijh[,,h]%*%w[,-1]+b})),c(g,m,r));
  xplusje=t(sapply(1:r,function(h){colSums(v_ijh[,,h])}))+e%*%t(rep(1,d))
  critere=lgamma(g*a)-g*lgamma(a)-lgamma(n+g*a)+sum(lgamma(nk+a))+
    ...
    sum(lgamma(xplusje[,w[,1]]))+
    g*m*(lgamma(r*b)-r*lgamma(b))+
    sum(lgamma(nkl+b))-sum(lgamma(Nkl+r*b))
  critere
}