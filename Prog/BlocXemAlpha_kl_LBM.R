...
BlocXemAlpha_kl_LBM =function(x_ij,theta,t_jl,a,b,niter=100000,eps=10^(-16),epsi=10^(-16),epsi_int=0.1,niter_int=1){
  ...
  for (iter in 1:niter){
    ...
    for (iter_int in 1:niter_int){
      # Computation of sik
      S_ik=(x_ij%*%t_jl)%*%t(A_kl)+rep(1,n)%*%t_l%*%(t(B_kl))+matrix(1,n,1)%*%t(log_pi_k);
    ...
    tau_l=t((t_l+a-1)/(sum(t_l)+m*(a-1)));
    if (any(is.nan(tau_l))){
      tau_l=t((t_l+a-1)/((sum(t_l)+m*(a-1))*((sum(t_l)+m*(a-1))>0)+
                               eps*((sum(t_l)+m*(a-1))<=0)));
    }
    u_kl=t(s_ik)%*%x_ij%*%t_jl;
    n_kl=t(s_k)%*%t_l;
    u_kl=u_kl*(u_kl>0);
    n_kl=n_kl*(n_kl>0);
    ...
    #Stopping criterion
    if ((max(c(abs(pi_old-pi_k),abs(tau_old-tau_l),abs(alpha_old-alpha_kl)))< epsi)){
      break
    }else{
      pi_old=pi_k
      tau_old=tau_l
  ...
  # if (is.na(W)){
    W=sum(c(s_k)*log(pi_k*(pi_k>0)+eps*(pi_k<=0)))+
      sum(c(t_l)*log(tau_l*(tau_l>0)+eps*(tau_l<=0)))+
